...
public class ParkService extends MementoService implements Park {
	...
	public ObjValue create(String domain, String node, byte[] obj, String sessionid, int auth, boolean heartbeat)
		...
		if (domain != null && node != null) {
			...
			try {
				...
				if (!parkinfo.containsKey(domainnodekey)) {
					...
					LogUtil.info("[Park]", "[create]", domainnodekey + " is exist!");// throw
																						// exist
																						// exception
			} catch (Exception e) {
				// e.printStackTrace();
			...
			try {
				...
				if (checkAuth(domainnodekey, sessionid, AuthPolicy.OP_READ_WRITE)) {
					if (parkinfo.containsKey(domainnodekey)) {
						...
						LogUtil.info("[Park]", "[update]", domainnodekey + " is not exist!");// throw
																								// not
																								// exist
																								// exception
				}
		...
		if (domain != null) {
			...
			try {
				if (parkinfo.containsKey(domain)) {
					...
					LogUtil.info("[Park]", "[update]", domain + " is not exist!");// throw
																					// not
																					// exist
																					// exception
			} catch (Exception e) {
	...
	protected ObjValue delete(String domain, String node) {
		...
		try {
			...
			if (!objrm.isEmpty()) {
				...
				objrm = null;
				LogUtil.info("[Park]", "[delete]", domainnodekey + " cant be deleted or not exist!");// throw
																										// not
																										// exist
																										// exception
			}
	...
	public boolean checkAuth(String domainnodekey, String sessionid, AuthPolicy targetauth) {
		...
		if (creator != null) {
			if (creator.equals(sessionid))
				...
				int thekeyauth = domainnodeAuth != null ? (Integer) domainnodeAuth : 1;// only
																						// read
																						// for
																						// domain
