...
abstract class ParallelService {
	...
	/*
	 * WorkerLocal[] getWorkersService(String workerType) { //LogUtil.fine("",
	 * "", "getWorkersService:"+workerType); List<ObjectBean> oblist =
	 * ParkPatternExector.getWorkerTypeList(workerType); List<WorkerLocal>
	 * wklist = new ArrayList<WorkerLocal>(); for(ObjectBean ob:oblist) {
	 * String[] hostport = ((String)ob.toObject()).split(":");
	 * wklist.add(BeanContext.getWorkerLocal(hostport[0],
	 * Integer.parseInt(hostport[1]), workerType)); } return wklist.toArray(new
	 * WorkerLocal[wklist.size()]); }
	 * 
	 * Workman[] getWorkersService(String host, int port, String workerType) {
	 * //LogUtil.fine("", "", "getWorkersService:"+workerType); List<ObjectBean>
	 * oblist = ParkPatternExector.getWorkerTypeList(workerType); List<Workman>
	 * wklist = new ArrayList<Workman>(); for(ObjectBean ob:oblist) { String[]
	 * hostport = ((String)ob.toObject()).split(":");
	 * if(!hostport[0].equals(host)&&!Integer.parseInt(hostport[1])!=port)
	 * wklist.add(BeanContext.getWorkman(hostport[0],
	 * Integer.parseInt(hostport[1]), workerType)); } return wklist.toArray(new
	 * WorkerLocal[wklist.size()]); }
	 */
	...
	List<String[]> getWorkersService(String host, int port, String workerType) {
		...
		List<ObjectBean> oblist = ParkPatternExector.getWorkerTypeList(workerType);// getWorkerTypeList(String
																					// host,
																					// int
																					// port,
																					// String
