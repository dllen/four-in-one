...
public final class FttpAdapter {
	...
	public FttpReadAdapter getFttpReader(long beginIndex, long bytesNum) throws FttpException {
		...
		/*
		 * FttpContractor fcread = fc.object(); fcread.setReadArea(beginIndex,
		 * bytesNum); return fcread;
		 */
	...
	public FttpWriteAdapter getFttpWriter(long beginIndex, long bytesNum) throws FttpException {
		 ...
		 * fcwrite.setWriteArea(beginIndex, bytesNum); return fcwrite;
		 */
	...
	/*
	 * public FileProperty[] getChildProperty() throws FttpException{
	 * FileResult[] frarr = fc.getChildProperty(); FttpContractor[] fcs = null;
	 * if(frarr!=null&&frarr.length>0){ fcs = new FttpContractor[frarr.length];
	 * for(int i=0;i<frarr.length;i++){ FttpContractor fc = getContractor();
	 * fc.acquireProperty(frarr[i]); fcs[i]=fc; } } return fcs; }
	 */
	...
	public static String[] fttpRootsPathEncode(String[] roots) {
		...
		if (rootspath != null) {
			for (int i = 0; i < rootspath.length; i++)
				rootspath[i] = ObjectBytes.getUtf8UrlString(rootspath[i]);// getUrlString
																			// encodeReplace
		}
		return rootspath;
	}
