...
class DumpCtor extends Contractor implements CoolHashBase, CoolHashClient {
	...
	public <T> T get(String key, Class<T> valueType, boolean point, String... pointSubKey) throws CoolHashException {
		...
		byte[] bts = writeAhead.getMark() && writeAhead.containsKey(key) ? (byte[]) writeAhead.get(key)
				: dump(key, null, (byte) 0x0, point, pointSubKey);
	...
	public int put(CoolHashMap keyvalue) {
		WareHouse[] wh = dump((String[]) keyvalue.keySet().toArray(new String[0]), keyvalue.getValues().toArray(), null,
				(byte) 0x2c, false);
		return getResultNum(wh);
	}
	...
	/*
	 * public void exit(){ ParkPatternExector.resetParkLocal(); super.exit();
	 * }//2014.10.17
	 */
