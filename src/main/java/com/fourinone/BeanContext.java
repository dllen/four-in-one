...
public class BeanContext extends ServiceContext {
	...
	public static ParkLocal getPark() {
		...
		return getPark(parkcfg[0][0], Integer.parseInt(parkcfg[0][1]), parkcfg);// input
																				// serverconfiglist
																				// string[][]
	}
	...
	public static void startPark(String host, int port, String sn, String[][] servers) {
		try {
			// DelegateConsole.bind(Park.class, new ParkService(host, port,
			// servers, sn));
			startService(host, port, sn, new ParkService(host, port, servers, sn));
			// BeanService.putBean(host, true, port, sn, new ParkService(host,
			// port, servers, sn));//input serverconfiglist string[][]
	...
	static WorkerLocal getFttpLocal(String host, int port, String sn) {
		return (WorkerLocal) DelegateConsole.bind(new Class[] { WorkerLocal.class, CtorLocal.class, FttpLocal.class },
	...
	/*
	 * public static <I extends Remote> void start(String host, int port, String
	 * sn, I i) { try{ BeanService.putBean(host,true,port,sn,i);
	 * }catch(RemoteException e){ e.printStackTrace(); } }
	 * 
	 * public static <I extends Remote> I get(Class<I> a, String host, int port,
	 * String sn){ I i=null; try{ i=(I)BeanService.getBean(host,port,sn);
	 * }catch(RemoteException e){ System.out.println(e); } return i; }
	 */

	public static void startCacheFacade(String host, int port) {
		try {
			startService(host, port, ConfigContext.getCacheFacadeService(), DelegateConsole.bind(Cache.class,
					new CacheFacade(ConfigContext.getCacheService(), ConfigContext.getCacheGroupConfig())));
		} catch (RemoteException e) {
			LogUtil.info("[BeanContext]", "[startCacheFacade]", e);
			// e.printStackTrace();
		}
		/*
		 * try{ BeanService.putBean(host,true,port,"CacheService",new
		 * CacheFacade());//input Groups }catch(RemoteException e){
		 * e.printStackTrace(); }
		 */
	}
	...
	public static Cache getCacheFacade() {// loadbalance server host and port
		 ...
		 * }catch(RemoteException e){ //e.printStackTrace();
		 * System.out.println(e); } return ca;
		 */
	}
	...
	/*
	 * public static Result<Integer> tryStart(String... params){ return
	 * tryStart(null, params); }
	 * 
	 * public static Result<Integer> tryStart(final FileAdapter fa, final
	 * String... params){ final Result<Integer> fr = new Result<Integer>(false);
	 * tpe().execute(new Runnable(){ public void run(){ try{ int r = start(fa,
	 * params); fr.setResult(new Integer(r)); fr.setReady(FileResult.READY);
	 * }catch(Throwable e){ LogUtil.info("[BeanContext]", "[tryStart]", e);
	...
	public static ObjectBean getLock(String locknode, String lockname) {
		ParkLocal pl = ParkPatternExector.getParkLocal();// String parkhost, int
															// parkport
	...
	/*
	 * private static ObjectBean getLock(String locknode, String lockname,
	 * Object lockvalue){//lockvalue=w,r ParkLocal pl =
	 * ParkPatternExector.getParkLocal();//getPark() ObjectBean ob =
	 * lockname!=null?pl.create(locknode, lockname,
	 * lockvalue):pl.create(locknode, lockvalue); String lockname =
	 * ob.getNode(); while(true){ List<ObjectBean> oblist = pl.get(locknode);
	 * String curnode = (String)oblist.get(0).getNode();
	 * if(curnode.equals(lockname)) break; } return ob; }
	 */
	...
	public static void startCoolHashServer(String parkhost, int parkport, int worknum, String jarclasspath,
			String logdir, String langCode) {
		startCoolHashServer("com.fourinone.CoolHashWorker", parkhost, parkport, worknum, jarclasspath, logdir,
				langCode);
	}
	...
	static void startCoolHashServerLocal(String parkhost, int parkport, int worknum, String jarclasspath,
			String logdir) {
		startCoolHashServer("com.fourinone.DumpWorker", parkhost, parkport, worknum, jarclasspath, logdir, null);
	}

	private static void startCoolHashServer(String service, String parkhost, int parkport, int worknum,
		...
		try {
			...
			starts[0] = BeanContext.tryStart("java", "-cp", jarclasspath, "com.fourinone.BeanContext", parkhost,
					parkport + "");
			...
			for (int i = 1; i < starts.length; i++) {
				if (langCode != null)
					starts[i] = BeanContext.tryStart("java", "-Dfile.encoding=" + langCode, "-cp", jarclasspath,
							service, parkhost, parkport + "", parkhost, (port++) + "");
				else
					starts[i] = BeanContext.tryStart("java", "-cp", jarclasspath, service, parkhost, parkport + "",
							parkhost, (port++) + "");
