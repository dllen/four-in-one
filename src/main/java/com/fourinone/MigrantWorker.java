...
public class MigrantWorker extends WorkerParallel implements ParkStatg {
	...
	protected Workman[] getWorkerElse(String workerType) {
		 ...
		 * if(parallelPatternFlag!=1) { List<String[]> wslist =
		 * getWorkersService(host, port, workerType); for(String[]
		 * wsinfo:wslist) wklist.add(BeanContext.getWorkerElse(wsinfo[0],
		 * Integer.parseInt(wsinfo[1]), wsinfo[2])); } return wklist.toArray(new
		 * Workman[wklist.size()]);
		 */
