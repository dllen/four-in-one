...
public class CoolHashMap extends LinkedHashMap implements Map, CoolHashBase {
	...
	/*
	 * static CoolKeySet<String> newKeySet(Collection c){ return new
	 * CoolHashMap().new CoolKeySet(c); }
	 */
	...
	private class DataMapEntry implements Map.Entry, Comparable<Map.Entry> {
		...
		public boolean equals(Object o) {
			if (o instanceof Map.Entry) {
				...
				if ((entry.getKey() == null ? k == null : entry.getKey().equals(k))
		...
		if (o instanceof CoolHashMap) {
			...
			if (size() == odm.size()) {
				...
				for (int i = 0; i < meset.length; i++) {
					...
					if (!(me.getKey().equals(odmme.getKey())
							&& Arrays.equals((byte[]) me.getValue(), (byte[]) odmme.getValue())))
						return false;
				}
				return true;
			}
